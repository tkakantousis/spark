...
class SparkSubmitUtilsSuite extends SparkFunSuite with BeforeAndAfterAll {
  ...
  test("create repo resolvers") {
    ...
    assert(res1.getResolvers.get(1).asInstanceOf[IBiblioResolver].getName === "kompics")
    assert(res1.getResolvers.get(2).asInstanceOf[FileSystemResolver].getName === "local-ivy-cache")
    assert(res1.getResolvers.get(3).asInstanceOf[IBiblioResolver].getName === "central")
    assert(res1.getResolvers.get(4).asInstanceOf[IBiblioResolver].getName === "spark-packages")
  }
